...
ORDER BY n.nspname, c.relname`
	...
	for rows.Next() {
		...
		// pg_get_viewdef emits the SELECT with a trailing ';'. The renderer adds
		// its own statement terminator (and, for matviews, a WITH [NO] DATA
		// clause before it), so strip the trailing ';' here to avoid an empty
		// statement / misplaced clause when the DDL is rendered.
		query := &irv1.SelectStmt{RawSql: strings.TrimRight(strings.TrimSpace(def), ";")}
...
//
// Functions with an extension ('e') or internal ('i') pg_depend row are
// excluded: extension functions belong to the extension, and internal
// dependencies mark functions auto-generated by another object (e.g. the
// range/multirange constructor functions PostgreSQL creates alongside a
// `CREATE TYPE ... AS RANGE`). Emitting either as a standalone CREATE FUNCTION
// would fail to apply (the function already exists once its owner is created).
