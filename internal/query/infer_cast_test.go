...
func TestInferExplicitParamCasts(t *testing.T) {
	...
	tests := []struct {
		...
		{"select", "SELECT @p::text AS value", "text", false},
		...
		{"insert_returning", "INSERT INTO users (col) VALUES ('x') RETURNING @p::text", "text", false},
		{"update_returning", "UPDATE users SET col = 'x' RETURNING @p::text", "text", false},
		{"delete_returning", "DELETE FROM users RETURNING @p::text", "text", false},
		{"case", "UPDATE users SET col = CASE WHEN id > 0 THEN @p::text ELSE col END", "text", false},
		...
		{"row", "SELECT ROW(@p::text, 1)", "text", false},
		{"subquery", "UPDATE users SET col = (SELECT @p::text)", "text", false},
		{"subquery_operand", "SELECT id FROM users WHERE @p::text IN (SELECT col FROM users)", "text", false},
		{"join", "SELECT u.id FROM users u JOIN users v ON v.col = @p::text", "text", false},
		{"from_function", "SELECT * FROM unnest(@p::text[])", "text[]", false},
		{"from_values", "SELECT v.col FROM (VALUES (@p::text)) AS v(col)", "text", false},
		{"values", "VALUES (@p::text)", "text", false},
		{"union", "SELECT 'x'::text UNION ALL SELECT @p::text", "text", false},
		{"cte_select", "WITH v AS (SELECT @p::text AS col) SELECT col FROM v", "text", false},
		{"cte_insert", "WITH v AS (SELECT @p::text AS col) INSERT INTO users (col) SELECT col FROM v", "text", false},
		{"cte_update", "WITH v AS (SELECT @p::text AS col) UPDATE users SET col = (SELECT col FROM v)", "text", false},
		{"cte_delete", "WITH v AS (SELECT @p::text AS col) DELETE FROM users WHERE col IN (SELECT col FROM v)", "text", false},
		...
		{"update_from", "UPDATE users SET col = v.col FROM (SELECT @p::text AS col) v", "text", false},
		{"delete_using", "DELETE FROM users USING (SELECT @p::text AS col) v WHERE users.col = v.col", "text", false},
		{"order_by", "SELECT id FROM users ORDER BY @p::text", "text", false},
		{"limit", "SELECT id FROM users LIMIT @p::bigint", "int8", false},
		{"offset", "SELECT id FROM users OFFSET @p::bigint", "int8", false},
		{"distinct_on", "SELECT DISTINCT ON (@p::text) id FROM users", "text", false},
		{"group_by", "SELECT count(*) FROM users GROUP BY @p::text", "text", false},
		{"having", "SELECT count(*) FROM users HAVING count(*) > @p::bigint", "int8", false},
		{"aggregate_filter", "SELECT count(*) FILTER (WHERE col = @p::text) FROM users", "text", false},
		{"aggregate_order", "SELECT array_agg(col ORDER BY @p::text) FROM users", "text", false},
		{"window_inline", "SELECT row_number() OVER (PARTITION BY @p::text ORDER BY id) FROM users", "text", false},
		{"window_named", "SELECT row_number() OVER w FROM users WINDOW w AS (ORDER BY @p::text)", "text", false},
		{"uuid", "SELECT @p::uuid", "uuid", false},
		{"jsonb", "SELECT @p::jsonb", "jsonb", false},
		{"array", "SELECT @p::text[]", "text[]", false},
